...
Продакшн-контур деградации:
  1) КАЧЕСТВО: сравнивает вчерашние прогнозы (predictions/forecast_*.csv) с фактом,
     считает MAPE/MASE по узлам. MASE > порога => модель хуже допуска.
  2) ДРЕЙФ ДАННЫХ: среднее/std нагрузки за последние 30 дней против обучающего
     распределения (manifest пишется при обучении) — грубый, но честный детектор
     "мир изменился, пора переобучаться".
  3) ФОЛБЭК: пишет status.json с решением use_fallback=true/false. Сервису/потребителю
     при use_fallback=true следует брать сезонный наив (тот же час неделю назад) —
     деградация мягкая, система не остаётся без прогноза.

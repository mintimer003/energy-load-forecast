...
Раз в сутки джоб строит прогноз на следующие 24 часа по каждому узлу и пишет
predictions/forecast_{дата}.csv (+ копия latest.csv для API). Модели не
переобучаются — загружаются из models/*.cbm.

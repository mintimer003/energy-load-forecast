...
**Тюнинг гиперпараметров** (`10_tuning.py`: Optuna, 30 проб, временная валидация +
early stopping) дал результат в пределах шума (±1 %): значения по умолчанию CatBoost
уже близки к оптимуму для этих данных — основной выигрыш дают данные и признаки, а
не тонкая настройка.
